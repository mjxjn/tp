...
class PurchaseAction extends CommonAction {
    ...
    /*public function all_dao($ids){
        foreach ($ids as $id){
            $this->allExcelPurchase($id,$ids);
        }
    }*/
    ...
    public function allExcelPurchase(){
        $id = $this->_get('id');
        if(empty($id)){
            $this->error("参数错误！");
        }
    ...
    public function outExcelPurchase(){
        $id = $this->_get('id');
        if(empty($id)){
            $this->error("参数错误！");
        }
        ...
        $sid=$Pinfo['sid'];
        $Supplier = D('Supplier');
        $Sinfo = $Supplier->where('flag=1 and sid="'.$sid.'"')->field('supplier')->find();
        ...
        $objExcel->getActiveSheet()->setCellValue('A1', '序');
        $objExcel->getActiveSheet()->setCellValue('B1', '条形码');
        $objExcel->getActiveSheet()->setCellValue('C1', '商品全名');
        $objExcel->getActiveSheet()->setCellValue('D1', '规格');
        $objExcel->getActiveSheet()->setCellValue('E1', '型号');
        $objExcel->getActiveSheet()->setCellValue('F1', '陈列');
        $objExcel->getActiveSheet()->setCellValue('G1', '装箱数量');
        $objExcel->getActiveSheet()->setCellValue('H1', '供货商');
        $objExcel->getActiveSheet()->setCellValue('I1', '补货数量');
        $objExcel->getActiveSheet()->setCellValue('J1', '到货情况');
        ...
        foreach ($Plist as $key=>$val){
           $objExcel->getActiveSheet()->setCellValue('A'.$i, $key+1);
            ...
            $objExcel->getActiveSheet()->setCellValue('D'.$i, $val['specification']);
            $objExcel->getActiveSheet()->setCellValue('E'.$i, $val['marque']);
            $objExcel->getActiveSheet()->setCellValue('F'.$i, $val['display']);
            $objExcel->getActiveSheet()->setCellValue('G'.$i, $val['box_num']);
            $objExcel->getActiveSheet()->setCellValue('H'.$i, $Sinfo['supplier']);
            $objExcel->getActiveSheet()->setCellValue('I'.$i, $val['goods_num']);
            $state = $val['get_goods_num']-$val['goods_num'];
            if($state>0){
                $state_flag = "已超出";
            }else if($state===0){
                $state_flag = "已到货";
            }else{
                $state_flag = $val['get_goods_num'];
            }
            $objExcel->getActiveSheet()->setCellValue('J'.$i, $state_flag);
           $i++; 
        }
        
        $outputFileName = $Sinfo['supplier']."-".$Pinfo['Warehouse']."-".  date("Ymd",time()).".xls";   
